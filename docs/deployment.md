...
Step-by-step guide for deploying the OpenClaw Dashboard on a headless Mac Mini with Tailscale.
...
2. [Step 1 — Mac Mini Setup](#step-1--mac-mini-setup)
3. [Step 2 — Install Tailscale on Mac Mini](#step-2--install-tailscale-on-mac-mini)
4. [Step 3 — Install & Configure OpenClaw](#step-3--install--configure-openclaw)
5. [Step 4 — Deploy the Dashboard](#step-4--deploy-the-dashboard)
6. [Step 5 — Expose via Tailscale Serve](#step-5--expose-via-tailscale-serve)
7. [Step 6 — Connect Clients](#step-6--connect-clients)
8. [Step 7 — oh-my-opencode Hook](#step-7--oh-my-opencode-hook)
9. [Step 8 — Verify Everything Works](#step-8--verify-everything-works)
10. [Step 9 — OpenClaw Cron Jobs (Optional)](#step-9--openclaw-cron-jobs-optional)
11. [Security Hardening Checklist](#security-hardening-checklist)
12. [Tailscale ACL Policy (Optional)](#tailscale-acl-policy-optional)
...
# OpenClaw Dashboard at /opencode
...
| `https://<hostname>.<tailnet>.ts.net/opencode` | OpenClaw Dashboard |
...
2. Open the app and **Sign in** with the same account (Google, GitHub, etc.)
...
openclaw cron add \
  ...
  --every 4h \
  --message "Check the OpenClaw Dashboard at http://127.0.0.1:3000/api/todos for any stale in_progress tasks or high-priority pending items. Summarize what you find." \
  --announce
```
...
4. If that works but the hostname doesn't, MagicDNS may be off. Enable it at [DNS settings](https://login.tailscale.com/admin/dns)
