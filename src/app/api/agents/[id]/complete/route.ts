...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    lifecycleManager.completeTask(params.id, data.taskId);
    ...
    console.error('Error completing lifecycle task:', error);
    return NextResponse.json({ error: 'Failed to complete task' }, { status: 500, headers: corsHeaders(request) });
  }
}
